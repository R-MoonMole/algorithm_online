...
#
#             # 시간을 +1 해주면서 기록
#             visited[new_y][new_x] = [now_y][now_x] + 1
...
# 1. 최소 벽돌
# - 현재 벽돌이 다 깨지면 더 이상 할 필요가 없다 -> 현재 벽돌수 관리

# N번의 구슬을 굴려야 한다
# - 모든 케이스를 보아야 한다 (12^4, 약 25만번)
#   - 백트래킹
#       - 한번 쏘았을 때 벽돌을이 연쇄로 깨진다.
#       - 현재 기준으로 퍼저나가면서 탐색(BFS)
#       - 빈칸 메우기
from collections import deque
...
def shoot(cnt, remains, now_arr):
    ...
    for col in range(W):
        ...
        # 방법1. 원본을 복사해두고, 재귀호출 갔다오면 다시 되돌리는 방법
        # 1. col 위치에 구슬을 떨어트리기 전 상태를 복사 (얕은 복사 주의)
        # 2. 원본 리스트의 col 위치에 떨어트리고
        # 3. cnt + 1 번 재귀 상태로 이동
        # 4. 떨구기 전 상태로 복구
        # -> 복사, 복구 시간이 엄청 오래걸림
